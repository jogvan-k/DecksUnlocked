...
namespace KeyforgeUnlockedConsole.ConsoleGames
{
    public abstract class BaseConsoleGame : IConsoleGame
    {
        ...
        public BaseConsoleGame(IState state, LogInfo logInfo = LogInfo.None)
        {
            _logInfo = logInfo;
            _state = state;
            _consoleWriter = new ConsoleWriter(state.Metadata.InitialDecks.SelectMany(d => d.Value)
        ...
        string ReadCommand()
        {
            while (true)
            {
                ...
                {
                    if (int.TryParse(command, out var i) && 0 < i &&
                        i <= Commands["action"].Actions(_state.ToImmutable()).Count)
                        return command;
                }
