...
namespace KeyforgeUnlockedConsole
{
    public static class Program
    {
        ...
        static IConsoleGame AiVsAiGame()
        {
            var player1Deck = DeckLoader.LoadDeck("Fyre, Bareleyhill Bodyguard.txt");
            var player2Deck = DeckLoader.LoadDeck("Fyre, Bareleyhill Bodyguard.txt");
            return new AIVsAIGame(StateFactory.Initiate(player1Deck, player2Deck),
                new NegamaxAI(new Evaluator(), MinimaxTypes.searchLimit.NewTurn(3, searchTime.NewSeconds(5)),
                    MinimaxTypes.SearchConfiguration.IncrementalSearch, MinimaxTypes.LoggingConfiguration.LogAll));
        }
    }
}