...
namespace KeyforgeUnlocked.Client
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var client = new AI.AIClient(channel);
            var reply = await client.SimulateAsync(
                new SimulateRequest { StateJson = "" });
            Console.WriteLine("Greeting: " + reply.MoveOrder);
