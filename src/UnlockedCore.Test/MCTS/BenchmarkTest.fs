...
type BenchmarkCases () =
    ...
    let evaluate evalFun n b =
        ...
        printfn "%i simulations performed in %i hours %i minutes and %i seconds %i milliseconds" logInfo.simulations logInfo.elapsedTime.Hours logInfo.elapsedTime.Minutes logInfo.elapsedTime.Seconds logInfo.elapsedTime.Milliseconds
        printfn "Best path: %s" (bestPath |> Array.map (string) |> Array.fold(fun s i -> if (s = "" ) then i else  s + ", " + i) "")
    ...
    member this.AllUniqueNodes () =
        ...
        let evalFun (d, h) =
            let playerTurn =
                if d = n then if h > totalEndNodes / 4 then p1 else p2
                else if d % 2 = 1 then p2 else p1
