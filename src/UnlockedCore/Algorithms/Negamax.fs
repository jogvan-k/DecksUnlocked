    ...
    | Node (actions) ->
        ...
        if (hashMapLookup.IsSome) then
            ...
            while (iter.MoveNext() && not acc.isTimeLimitReached) do
                ...
                if (alpha' < beta) then
                    let candidate =
                        nextCandidate alpha' beta color a depth acc tTable p

                    if (fst candidate) > (fst currentBest) then
                        currentBest <- (fst candidate, i :: snd candidate)
...
and nextCandidate alpha beta color (action: ICoreAction) depth acc tTable pv =
    ...
    if (changingPlayer action.Origin nextState) then
        recNegamax (-1 * beta) (-1 * alpha) (-color) nextDepth nextState acc tTable pv
        |> flipValue
    else
...
let negamax (depth: remainingSearch) (s: ICoreState) (acc: accumulator) pv =
    ...
    let transpositionTable =
        new transpositionTable (acc.logConfig, acc)

    let result =
        recNegamax -Int32.MaxValue Int32.MaxValue (startColor s) depth s acc transpositionTable pv

    acc.successfulHashMapLookups <-
        (transpositionTable :> ITranspositionTable)
            .successfulLookups
