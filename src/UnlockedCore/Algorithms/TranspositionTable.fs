...
let logSuccessfulHashMapLookup (logConfig: LoggingConfiguration) =
    logConfig.HasFlag(LoggingConfiguration.LogSuccessfulHashMapLookup)

let avoidHashMap (searchConfig: SearchConfiguration) =
    searchConfig.HasFlag(SearchConfiguration.NoHashTable)
...
type transpositionTable(logConfig: LoggingConfiguration, ?acc: accumulator) =
    ...
    let _lookupHashMap: int -> (int * int list) option =
        if (logSuccessfulHashMapLookup logConfig) then
            fun hash ->
                let lookup = _hashMap.TryFind(hash)

                if (lookup.IsSome) then
                    _lookupIncrement 1

                lookup
        else
            fun hash -> _hashMap.TryFind(hash)

    let _addToHashMap =
        if (avoidHashMap searchConfig) then
            fun (_, _) -> ()
        else
            fun (hash, value) -> _hashMap <- _hashMap.Add(hash, value)
