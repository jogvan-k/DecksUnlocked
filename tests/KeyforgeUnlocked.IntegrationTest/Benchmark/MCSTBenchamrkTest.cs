...
namespace KeyforgeUnlocked.IntegrationTest.Benchmark
{
    ...
    sealed class MCSTBenchamrkTest
    {
        ...
        public void NumberOfSimulations()
        {
            ...
            var ai = new UnlockedCore.MCTS.AI.MonteCarloTreeSearch(searchTime.NewSeconds(10), 1000000,
                UnlockedCore.MCTS.AI.configuration.AsyncExecution);

            ((IGameAI)ai).DetermineAction(root);

            Console.WriteLine(
                $"Ran {ai.LatestLogInfo().simulations} simulations in {ai.LatestLogInfo().elapsedTime.Seconds} seconds.");
        }
