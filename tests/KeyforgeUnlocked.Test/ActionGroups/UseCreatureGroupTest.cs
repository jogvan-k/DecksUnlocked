    ...
    sealed class UseCreatureGroupTest
    {
        ...
        public void Actions_OpponentWithTaunt_AttackActionsOnlyOnValidCreatures()
        {
            ...
            var field = TestUtil.Lists(
                ...
                new[]
                {
                    _opponentCreature1, _opponentCreatureWithTaunt1, _opponentCreatureWithTaunt2, _opponentCreature2,
                    _opponentCreature3
                });
            var state = StateTestUtil.EmptyState.New(fields: field).ToImmutable();
            var sut = new UseCreatureGroup(state, creature);
            ...
            [Values(false, true)] bool creatureStunned)
        {
            var creature = new Creature(_sampleCreatureCardWithCreatureAbility, isReady: true,
                state: creatureStunned ? CreatureState.Stunned : CreatureState.None);
            var state = SetupState(creature);
        ...
        public void Actions_ConditionalActionAllowed(
            [Values(typeof(FightCreature), typeof(Reap), typeof(UseCreatureAbility))]
            Type type)
        {
            ActionPredicate actionPredicate = (_, a) => a.GetType().IsAssignableTo(type);
            var creatureCard =
                new SampleCreatureCard(creatureAbility: Delegates.NoChange, useActionAllowed: actionPredicate);
        ...
        {
            var creatureCard =
                new SampleCreatureCard(creatureAbility: Delegates.NoChange, useActionAllowed: (_, _) => false);
            var creature = new Creature(creatureCard, isReady: true);
