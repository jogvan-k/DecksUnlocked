...
namespace KeyforgeUnlockedTest.Effects.FightCreatureTests
{
    ...
    sealed class ElusiveKeywordTest
    {
        ...
        public void Resolve_TargetIsElusiveAndAttackedPreviousTurn_NoDamageDealt()
        {
            ...
            {
                CreaturesAttackedThisTurn = ImmutableHashSet<IIdentifiable>.Empty.Add(new Identifiable(targetCreature))
            };
            var expectedState = startState.Extend(resolvedEffects: new LazyList<IResolvedEffect>(resolvedEffects),
                fields: expectedFields, historicData: expectedHistoricData);
        ...
        public void Resolve_TargetIsElusiveAndAttackedCurrentTurn_ElusiveCreatureDead()
        {
            ...
            var fields = TestUtil.Lists(new[] { foughtCreature, fightingCreature }.AsEnumerable(),
                new[] { targetCreature });
            var startState = StateTestUtil.EmptyState.New(
                    turnNumber: 1,
                    fields: fields,
                    ...
                    historicData: new LazyHistoricData
                    {
                        CreaturesAttackedThisTurn =
                            ImmutableHashSet<IIdentifiable>.Empty.Add(new Identifiable(targetCreature))
                    })
                .Extend().ToImmutable();
            var state = startState.ToMutable();
            var sut = new FightCreature(fightingCreature, targetCreature);
            ...
            var expectedField = TestUtil.Lists(new[] { foughtCreature, new Creature(fightingCreatureCard, damage: 2) },
            ...
            {
                new CreatureFought(expectedFightingCreature, expectedTargetCreature),
                new CreatureDied(expectedTargetCreature)
            };
            var expectedDiscards = TestUtil.Sets(Enumerable.Empty<ICard>(), new[] { targetCreatureCard });
            var expectedHistoricData = new LazyHistoricData
            {
                EnemiesDestroyedInAFightThisTurn = 1,
                CreaturesAttackedThisTurn = ImmutableHashSet<IIdentifiable>.Empty.Add(new Identifiable(targetCreature))
            };
            var expectedState =
                startState.Extend(resolvedEffects: new LazyList<IResolvedEffect>(expectedResolvedEffects),
                    fields: expectedField, discards: expectedDiscards, historicData: expectedHistoricData);
            StateAsserter.StateEquals(expectedState, state);
        }
    }
}