...
namespace KeyforgeUnlockedTest.States
{
    ...
    sealed class CardControlTest
    {
            ...
            Player player)
        {
            ...
            var expectedState = StateTestUtil.EmptyState.New(purgedCards: expectedPurgedCards,
                resolvedEffects: expectedResolvedEffects);
            StateAsserter.StateEquals(expectedState, state);
        }
            ...
            [Range(0, 1)] int cardNo)
        {
            ...
            var expectedState = StateTestUtil.EmptyState.New(discards: expectedDiscards,
                purgedCards: expectedPurgedCards, resolvedEffects: expectedResolvedEffects);
            ...
            [Range(0, 1)] int cardNo)
        {
            ...
            StateAsserter.StateEquals(
                StateTestUtil.EmptyState.New(hands: expectedHands, archives: expectedArchives,
                    resolvedEffects: resolvedEffects), state);
        }
        ...
        public void PopArchive_NoCardsInArchive_NoEffect([Values(Player.Player1, Player.Player2)] Player playerTurn)
        {
            ...
            var state = StateTestUtil.EmptyState.New(playerTurn: playerTurn, archives: archives,
                hands: SetupCardSets());
            ...
            var expectedState = StateTestUtil.EmptyState.New(playerTurn: playerTurn, archives: expectedArchives,
                hands: SetupCardSets());
            StateAsserter.StateEquals(expectedState, state);
        }
        ...
        public void PopArchive([Values(Player.Player1, Player.Player2)] Player playerTurn)
        {
            ...
            StateAsserter.StateEquals(
                StateTestUtil.EmptyState.New(playerTurn: playerTurn, archives: expectedArchives, hands: expectedHands,
                    resolvedEffects: expectedResolvedEffects), state);
        }
            ...
            Player player)
        {
            ...
            var expectedState = StateTestUtil.EmptyState.New(playerTurn: player, decks: expectedDecks,
                discards: expectedDiscards, resolvedEffects: expectedResolvedEffects);
            StateAsserter.StateEquals(expectedState, state);
        }
            ...
            Player player)
        {
            ...
            var expectedResolvedEffects =
                new LazyList<IResolvedEffect>(new[] { (IResolvedEffect)new DiscardShuffledIntoDeck(player) });
            var expectedState = StateTestUtil.EmptyState.New(player, decks: expectedDecks, discards: expectedDiscards,
                resolvedEffects: expectedResolvedEffects, historicData: expectedHistoricData, metadata: metadata);
