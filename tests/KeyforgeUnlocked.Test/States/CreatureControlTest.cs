...
namespace KeyforgeUnlockedTest.States
{
    ...
    sealed class StateExtensions
    {
        ...
        public void ReturnCreatureToHand([Values(Player.Player1, Player.Player2)] Player player)
        {
            ...
            events.Subscribe(new Identifiable(""), EventType.CreatureReturnedToHand,
                (_, _, _) => creatureReturnedToHandEventInvoked = true);
            ...
            [Values(true, false)] bool targetAlreadyStunned)
        {
            var targetCreatureCard = new SampleCreatureCard();
            var targetCreature = new Creature(targetCreatureCard,
                state: targetAlreadyStunned ? CreatureState.Stunned : CreatureState.None);
            var otherCreature = new Creature(new SampleCreatureCard());
            ...
            var expectedFields =
                InstantiateFields(Player.Player1, new Creature(targetCreatureCard, state: CreatureState.Stunned),
                    otherCreature);
        ...
        static ImmutableLookup<Player, IMutableList<Creature>> InstantiateFields(Player player, Creature targetCreature,
