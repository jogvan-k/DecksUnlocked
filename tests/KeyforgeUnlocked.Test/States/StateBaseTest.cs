...
namespace KeyforgeUnlockedTest.States
{
    ...
    sealed class StateBaseTest
    {
        ...
        readonly IImmutableDictionary<Player, IMutableSet<ICard>> _simpleSet =
        ...
        readonly IImmutableDictionary<Player, IMutableStackQueue<ICard>> _simpleStack =
            new Dictionary<Player, IMutableStackQueue<ICard>>
            {
                { Player.Player1, new LazyStackQueue<ICard>() },
                { Player.Player2, new LazyStackQueue<ICard>(new[] { new SampleCreatureCard() }) }
            }.ToImmutableDictionary();

        readonly IImmutableDictionary<Player, IMutableList<Creature>> _simpleField =
        ...
        public void Equals_EmptyAndNonEmpty()
        {
            ...
            Assert.False(emptyState.New(actionGroups: new LazyList<IActionGroup> { new EndTurnGroup() })
                .Equals(emptyState));
            ...
            Assert.False(emptyState.New(effects: new LazyStackQueue<IEffect>(new[] { new EndTurn() }))
                .Equals(emptyState));
            Assert.False(emptyState.New(resolvedEffects: new LazyList<IResolvedEffect> { new TurnEnded() })
                .Equals(emptyState));
            Assert.False(
                emptyState.New(
