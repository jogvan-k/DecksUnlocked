...
type BackPropagatingTest() =
    ...
    let branchingNode =
        nb (
            ...
            0,
            [ nb(p1, 1, 0, 1)
                .addChild(nb (p2, 2, 0, 11))
                .addChild (nb (p1, 3, 0, 111))
              nb(p2, 1, 0, 2)
                  .addChild(nb (p1, 2, 0, 22))
                  .addChild (nb (p2, 3, 0, 222))
              nb(p1, 1, 0, 3)
                  .addChild(nb (p2, 2, 0, 33))
                  .addChild (nb (p1, 3, 0, 333)) ]
        )
    ...
    member this.BranchingTree([<Values(0, 1, 2)>] branch, [<Values(Player.Player1, Player.Player2)>] playerWin) =
        ...
        let action2 =
            Action(state2.playerTurn, getLeaf (state2.leaves.[0]))
