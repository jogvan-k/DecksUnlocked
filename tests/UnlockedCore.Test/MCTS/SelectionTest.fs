...
type selectionTests() =
    let branchingNode =
        nb(p1, 0, 0, 0)
            .addChildren(
                [ nb (p2, 0, 0, 1, nb (p2, 0, 0, 4, nb (p2, 0, 0, 7)))
    ...
    member this.ExploredAndUnexploredLeaves_SelectUnexplored() =
        ...
        root.leaves <-
            [| Leaf.Leaf(Action(root.playerTurn, State(branchingNode.children.[0])))
               Leaf.Terminal(p2)
    ...
    member this.AllExploredOnRoot_SelectHighestEvaluated([<Range(1, 3)>] highestEvaluated: int) =
        ...
        root.leaves <-
            ...
            |> Array.mapi
                (fun i -> fun _ -> Leaf.Leaf(Action(branchingNode.playerTurn, State(branchingNode.children.[i]))))
    ...
    [<TestCase(2, 5)>]
    [<TestCase(3, 6)>]
    member this.AllExploredOnRootAndOneLevelDown_CandidateRecursivelyChosen
        (
