...
type nb(playerTurn, turnNumber, value, hash, children: nb list) =
    ...
    member this.build(?parent) =
        let node =
            node (playerTurn, turnNumber, value, hash, parent)

        node.children <-
            children
            |> List.map (fun (c: nb) -> c.build (node))
...
type evaluator() =
    interface IEvaluator with
        member this.Evaluate s = (s :?> node).value
